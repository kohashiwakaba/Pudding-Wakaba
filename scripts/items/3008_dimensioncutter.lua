wakaba.TRINKET_DIMENSION_CUTTER = Isaac.GetTrinketIdByName("Dimension Cutter")
...
function wakaba:NewRoom_DimensionCutter()
	if not Game():GetRoom():IsClear() then
		for i = 1, Game():GetNumPlayers() do
			...
			if player:HasTrinket(wakaba.TRINKET_DIMENSION_CUTTER) then
				for i = 1, player:GetTrinketMultiplier(wakaba.TRINKET_DIMENSION_CUTTER) do
					local rng = player:GetTrinketRNG(wakaba.TRINKET_DIMENSION_CUTTER)
...
function wakaba:TearCollision_DimensionCutter(tear, entity, low)
	...
	or entity.Type == EntityType.ENTITY_BEAST and entity.Variant == 0 then
		...
		if spawner and spawner:ToPlayer() and spawner:ToPlayer():HasTrinket(wakaba.TRINKET_DIMENSION_CUTTER) then
