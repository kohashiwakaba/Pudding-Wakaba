wakaba.COLLECTIBLE_EDEN_STICKY_NOTE = Isaac.GetItemIdByName("Eden's Sticky Note")
local playerType = 30
...
function wakaba:ItemUse_EdenNote(_, rng, player, useFlags, activeSlot, varData)
	...
	player:RemoveCollectible(wakaba.COLLECTIBLE_EDEN_STICKY_NOTE)
	...
	if not (useFlags & UseFlag.USE_NOANIM == UseFlag.USE_NOANIM) then
		player:AnimateCollectible(wakaba.COLLECTIBLE_EDEN_STICKY_NOTE, "UseItem", "PlayerPickup")
	end
...
wakaba:AddCallback(ModCallbacks.MC_PRE_USE_ITEM, wakaba.ItemUse_EdenNote, wakaba.COLLECTIBLE_EDEN_STICKY_NOTE)
