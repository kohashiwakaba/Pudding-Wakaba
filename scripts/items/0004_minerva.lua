...
function wakaba:initAura(player)
	...
	player:GetData().wakaba.aura = EntityRef(aura)
	return aura
end
...
function wakaba:PlayerUpdate_Minerva(player)
	...
	if wakaba:hasAura(player) and not player:GetData().wakaba.aura then
		local aura = wakaba:initAura(player)
	end
	...
	--[[ for i = 1, wakaba.state.storedplayers do
		...
		for s = 1, wakaba.state.storedplayers do
...
function wakaba:Update_Minerva()
	...
	for _, entity in ipairs(entities) do
		if wakaba.state.hasnemesis and not entity:HasEntityFlags(EntityFlag.FLAG_BRIMSTONE_MARKED) then
...
function wakaba:PlayerTakeDmg_Minerva(entity, amount, flag, source, countdownFrames)
	...
	and (entity:GetData().wakaba and entity:GetData().wakaba.hasminerva > 0)
	then
		local rng = wakaba.RNG:RandomFloat() * 100
		Isaac.DebugString(rng)
		if rng <= 16 or entity:GetData().wakaba.minervadmgprotect > 0 then
