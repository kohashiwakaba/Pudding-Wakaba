  ...
  for i = 1, maxID do
    ...
    then
      ...
      elseif bookstate == wakaba.bookstate.BOOKSHELF_SHIORI_DROP then
        if wakaba:unlockCheck(item.ID) then
          if (wakaba.shioribookblacklisted[item.ID] and wakaba.shioribookblacklisted[item.ID][bookstate]) or wakaba:has_value(wakaba.state.shioridropped, item.ID) then
            -- Do nothing
          else
            table.insert(books,item.ID)
            Isaac.DebugString("[wakaba]Book " .. item.Name .. " Inserted")
...
function wakaba:updateShiori(player)
	if player:GetPlayerType() == playerType and Game().Challenge == Challenge.CHALLENGE_NULL then
    ...
    if wakaba.state.currentshiorimode == wakaba.shiorimodes.SHIORI_COLLECTOR then
...
function wakaba:ItemUse_Shiori(useditem, rng, player, useflag, slot, vardata)
  ...
  if player:GetPlayerType() == playerType and slot ~= -1 then
    ...
    if wakaba.state.currentshiorimode == wakaba.shiorimodes.SHIORI_CURSE_OF_SATYR then
      local data = player:GetData()
...
function wakaba:PostShioriPickupCollision(pickup, collider, low)
  if collider:ToPlayer() ~= nil then
    local player = collider:ToPlayer()
      ...
      and wakaba.state.currentshiorimode == wakaba.shiorimodes.SHIORI_PURE_BODY then
...
function wakaba:PostShioriPickupInit(pickup)
...
--wakaba:AddCallback(ModCallbacks.MC_POST_PICKUP_INIT, wakaba.PostShioriPickupInit)
...
function wakaba:NewLevel_Shiori()
  for i = 1, Game():GetNumPlayers() do
    ...
    and (wakaba.state.currentshiorimode == wakaba.shiorimodes.SHIORI_AKASIC_RECORDS and Game().TimeCounter > 0) then
      if Game().Challenge == Challenge.CHALLENGE_NULL then
        local data = player:GetData()
...
wakaba:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, wakaba.onShioriCache)
