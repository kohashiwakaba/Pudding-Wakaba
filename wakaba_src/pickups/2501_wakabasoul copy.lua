	...
	if CurStage ~= LevelStage.STAGE8 then
		...
		while #candidates > 0 do
			...
			if replacedIdx then
				...
				local copyIdx = -1
				local tempRoom = wakaba.G:GetLevel():GetRoomByIdx(-1,-1)
				...
				if card == wakaba.Enums.Cards.SOUL_WAKABA2 then
					table.insert(wakaba.levelstate.wakabadevilshops, selected)
				else
					table.insert(wakaba.levelstate.wakabaangelshops, selected)
				end
				candidates = {}
