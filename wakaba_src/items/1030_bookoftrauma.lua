...
function wakaba:ItemUse_BookOfTrauma(_, rng, player, useFlags, activeSlot, varData)
	...
	local isGolden = Epiphany and Epiphany.API and Epiphany.API:IsGoldenItem(item)
	...
	if #entities > 0 then
		for _, e in ipairs(entities) do
			if i < max and (allowOtherTears or GetPtrHash(isc:getPlayerFromEntity(e)) == GetPtrHash(player)) then
				wakaba.G:BombExplosionEffects(e.Position, player.Damage * 8 * (isGolden and 1.3 or 1) + 20, TearFlags.TEAR_BRIMSTONE_BOMB | (isGolden and TearFlags.TEAR_GIGA_BOMB or 0), Color.Default, player, (isGolden and 2 or 1), true, false, DamageFlag.DAMAGE_EXPLOSION | DamageFlag.DAMAGE_IGNORE_ARMOR)
				e:Remove()
				i = i + 1
			elseif i >= max then
				finished = true
