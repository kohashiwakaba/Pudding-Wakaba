	...
	if shouldActivateReversal() then
		if sprite:IsEventTriggered("DropSound") then
			...
			if trinket.SubType > 32768 then
				...
				for i = 1, 2 do
					local newPos = Isaac.GetFreeNearPosition(trinket.Position, 40.0)
					Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, wakaba.G:GetItemPool():GetCollectible(ItemPoolType.POOL_PLANETARIUM, true, s), newPos, Vector.Zero, nil)
				end
				trinket:Remove()
			else
				sfx:Play(SoundEffect.SOUND_SOUL_PICKUP, 1, 0, false, 1)
