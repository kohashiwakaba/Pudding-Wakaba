...
wakaba:saveDataManager("Wakaba Gamblerun", pedestal_data)
...
function wakaba:PickupCollision_GambleRun(pickup, colliders, low)
	...
	if DifficultyManager and DifficultyManager.GetDifficulty() == "Gamble" and not wakaba:isGamblePedestalAllowed(pickup) then
...
function wakaba:PostRestock_GambleRun(partial)
	if not (DifficultyManager and DifficultyManager.GetDifficulty() == "Gamble") then return end
