...
wakaba.SpriteFromProfile = {
  ["Debug"] = function() return wakaba.sprites.LunarChargeSprite end,
  ["RushPony"] = function() return wakaba.sprites.WhitePonySprite end,
  ["DeliveryMinerva"] = function() return wakaba.sprites.DeliveryMinervaSprite end,
  ["LunarGauge"] = function() return wakaba.sprites.LunarChargeSprite end,
  ["FlashShift"] = function() return wakaba.sprites.FlashShiftSprite end,
  ["ConcentrationDanger"] = function() return wakaba.sprites.ConcentrationDangerSprite end,
  ["ConcentrationLunar"] = function() return wakaba.sprites.ConcentrationLunarSprite end,
  ["Concentration"] = function() return wakaba.sprites.ConcentrationSprite end,
  ["PhantomCloak"] = function() return wakaba.sprites.PhantomCloakSprite end,
}
...
function wakaba:RenderChargeBar(player, chargeno, profile, currentvalue, minvalue, maxvalue, spriteName, offset, includefinishanim, reverse, count, countprefix, countsubfix)
  if not player then return end
  if not profile then return end
  spriteName = spriteName or profile
  local sprite = wakaba.SpriteFromProfile[spriteName] and wakaba.SpriteFromProfile[spriteName]()
...
function wakaba:PlayerRender_ChargeBar(player)
  ...
  if chargestate and #chargestate > 0 then
    for i, e in ipairs(chargestate) do
      if e then
        local c = e
        wakaba:RenderChargeBar(player, c.Index, c.Profile, c.CurrentValue, c.MinValue, c.MaxValue, c.Sprite, wakaba:GetChargebarPos(i), c.IncludeFinishAnim, c.Reverse, c.Count, c.CountPrefix, c.CountSubfix)
      end
    end
  end
end
