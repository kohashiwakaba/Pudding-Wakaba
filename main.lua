...
wakaba.version = "v97 Tsukasa 2022.04.08"
wakaba.intversion = 9700
...
function wakaba:PreTakeDamage_GrimreaperDefender(entity, amount, flag, source, countdown)
	if entity.Type == EntityType.ENTITY_PLAYER then
		local player = entity:ToPlayer()
		if not player:GetData().wakaba then return end
		if player:GetData().wakaba.grimreaper then
			--print("Grimreaper damage")
			if amount <= 1 
			and flag & DamageFlag.DAMAGE_NOKILL == DamageFlag.DAMAGE_NOKILL 
			...
			if Game():GetRoom():GetType() == RoomType.ROOM_SACRIFICE
			and flag & DamageFlag.DAMAGE_SPIKES == DamageFlag.DAMAGE_SPIKES 
			then
				return
			end
			amount = 1
			entity:TakeDamage(amount, flag | DamageFlag.DAMAGE_NOKILL | DamageFlag.DAMAGE_RED_HEARTS | DamageFlag.DAMAGE_NO_PENALTIES, source, countdown)
			return false
		end
  end
end
--wakaba:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, wakaba.PreTakeDamage_GrimreaperDefender)

function wakaba:PreTakeDamage_DeterminationRibbon(entity, amount, flag, source, countdown)
	if entity.Type == EntityType.ENTITY_PLAYER then
		if Game():GetRoom():GetType() == RoomType.ROOM_SACRIFICE
		and flag & DamageFlag.DAMAGE_SPIKES == DamageFlag.DAMAGE_SPIKES 
		then
		...
		local player = entity:ToPlayer()
		if player:HasTrinket(wakaba.TRINKET_DETERMINATION_RIBBON) then
			if amount <= 1 and flag & DamageFlag.DAMAGE_NOKILL == DamageFlag.DAMAGE_NOKILL then return end
			player:TakeDamage(1, flag | DamageFlag.DAMAGE_NOKILL, source, 0)
			return false
		end
  end
end
--wakaba:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, wakaba.PreTakeDamage_DeterminationRibbon)

function wakaba:TakeDmg_BookOfTheGod(entity, amount, flag, source, countdownFrames)
	if not entity:GetData().wakaba then return end
	if entity.Type == EntityType.ENTITY_PLAYER
	then
		local player = entity:ToPlayer()
		if player:HasCollectible(CollectibleType.COLLECTIBLE_HEARTBREAK) then return end
		if flag & DamageFlag.DAMAGE_NOKILL == DamageFlag.DAMAGE_NOKILL then return end
		if player:GetData().wakaba.shioriangel then
			player:TakeDamage(1, flag | DamageFlag.DAMAGE_NOKILL, source, countdownFrames)
			player:AddBrokenHearts(1)
			return false
		end
	end
end
--wakaba:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, wakaba.TakeDmg_BookOfTheGod)
